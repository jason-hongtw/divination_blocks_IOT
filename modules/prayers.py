...
def start_throw():
    ...
    try:
        ...
        print(f"[ERROR] 無法發送擲杯指令: {e}")
        current_status = PENDING
        return jsonify({"status": "error", "message": "ESP-32S 擲杯指令發送失敗"}), 500

    return jsonify({"status": "processing", "message": "擲杯中"}), 200
...
def set_result(predicted_result):
    ...
    try:
        ...
        print(f"[INFO] 擲杯結果已傳送至 ESP-32S: {result}")
    except requests.exceptions.RequestException as e:
        print(f"[ERROR] 無法傳送擲杯結果至 ESP-32S: {e}")

    return jsonify({"status": "success", "message": f"擲杯結果設定為 {result}"}), 200
