...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (typeof checkInput === 'function') {
        ...
        document.getElementById("userBirth").addEventListener("input", checkInput);
        document.getElementById("userAddress").addEventListener("input", checkInput);
        // 如果 checkInput 也處理 userQuestion，下面這行可能多餘或需要調整邏輯
        // document.getElementById("userQuestion").addEventListener("input", checkInput);
    }
    // 監聽 userQuestion 輸入，以重置 '是否能問' 的狀態
    if (typeof resetCanAskState === 'function') { // <-- 已修正縮排
        document.getElementById("userQuestion").addEventListener("input", resetCanAskState);
    }


    // 初始化攝影機 (呼叫來自 camera_setup.js 的函數)
    if (typeof initializeCamera === 'function') { // 同樣建議加上檢查
        initializeCamera();
    }
